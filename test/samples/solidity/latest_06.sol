...
contract SampleBase is SampleAbstract(1) {
    ...
    uint immutable immutableVar1 = 1;
    ...
    function internalCallback() pure internal {}
    ...
    function testSlices() pure public {
    ...
    function testTryCatch() alertingAfter("Other contract creation") public {
        try new Empty() {
            int a = 1;
        } catch  {
            int b = 2;
    ...
    function basicFunctionality() onlyPositiveSomeBefore() internal returns (uint) {
        function(address) internal returns (address payable) converter = SampleBase.abstractFunc;
        function() pure sel = SampleBase.internalCallback;
